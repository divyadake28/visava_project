...
<div class="max-w-4xl mx-auto bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8">
    <div class="mb-6 flex items-center justify-between border-b border-slate-100 pb-4">
        <div>
            <h2 class="text-lg font-bold text-slate-900">Edit Event Information</h2>
            <p class="text-xs text-slate-500">Update event date, locations and bilingual descriptions</p>
        </div>
    ...
    <form action="{{ route('admin.events.update', $event) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
            ...
            <button type="submit" class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl shadow-sm transition">Update Event</button>
        </div>
    </form>
</div>
@endsection
